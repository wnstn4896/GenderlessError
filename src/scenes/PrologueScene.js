...
export class PrologueScene extends Phaser.Scene {
    ...
    onNextDialogue() {
        // '사건 현장에 가본다' 선택 시
        if (this.choiceResult === "going" && this.currentIndex === 14) {
            this.currentIndex = 16; // 바로 마지막 대사로 이동
        }
        
        // '가지 않는다' 선택 시
        if (this.choiceResult === "notGoing" && this.currentIndex === 16) {
            this.endScene(); // 마지막 대사 출력 없이 종료
            return;
        }
    ...
    endScene() {
        if (this.choiceResult === "going") {
            // '사건 현장에 가본다' 선택 시 VisualNovelScene으로 이동
            this.scene.stop('PrologueScene');
            this.scene.start('EarlyPartScene');
        } else if (this.choiceResult === "notGoing") {
            // '가지 않는다' 선택 시 새로고침하여 타이틀 화면 이동
